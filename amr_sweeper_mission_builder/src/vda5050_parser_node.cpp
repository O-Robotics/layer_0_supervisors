...
void Vda5050MissionParser::loadMission(const Vda5050MissionBuildConfig & config)
{
  ...
  const std::filesystem::path order_path =
    packageOrderPath(resolveMissionPath(config.mission_path));
...
void Vda5050MissionParser::loadFromLegacyGeoJson(const nlohmann::json & document)
{
  for (const auto & feature : document.at("features")) {
    ...
    projectAndStoreZone(parseOuterRing(feature.at("geometry")), zone_name,
        normalizeZoneType(zone_type));
  }
}
...
void Vda5050MissionParser::loadFromVda5050Order(const nlohmann::json & document)
{
  ...
  for (const auto & map_id : order_map_ids_) {
    if (map_georeferences_.find(map_id) == map_georeferences_.end()) {
      throw std::runtime_error("VDA5050 order references mapId without map_georeference: " +
          map_id);
    }
  }
...
void Vda5050MissionParser::loadMapGeoreference(const std::filesystem::path & order_path)
{
  ...
  const auto maps_document =
    document.contains("maps") ? document.at("maps") : nlohmann::json::array({document});
  ...
  for (const auto & map_document : maps_document) {
    ...
    if (georeference.units != "m") {
      throw std::runtime_error("map_georeference units must be \"m\" for mapId " +
          georeference.map_id);
    }
    ...
    {
      throw std::runtime_error("map_georeference bounds are empty for mapId " +
          georeference.map_id);
    }
...
void Vda5050MissionParser::validateVda5050Order(const nlohmann::json & document) const
{
      ...
      "workingZones", "noGoZones", "mission_type"})
  {
    if (document.contains(forbidden_key)) {
      throw std::runtime_error(std::string("VDA5050 order contains non-compliant field: ") +
          forbidden_key);
    }
  }
...
{
  if (!document.is_object() || !document.contains("zoneSet") ||
    !document.at("zoneSet").is_object())
  {
  ...
  for (const auto & zone : zone_set.at("zones")) {
    ...
    for (const auto & vertex : zone.at("vertices")) {
      if (!vertex.is_object() || !isFiniteNumber(vertex.at("x")) ||
        !isFiniteNumber(vertex.at("y")))
      {
...
void Vda5050MissionParser::saveMissionWaypointsArtifact(const std::string & path) const
{
  ...
  for (const auto & waypoint : mission_waypoints_) {
    ...
    if (projection_initialized_) {
      ...
      double altitude = 0.0;
      projector_.Reverse(waypoint.map_point.x, waypoint.map_point.y, 0.0, latitude, longitude,
          altitude);
...
void Vda5050MissionParser::clearCoveragePathCorridor(RasterizedMap & map) const
{
  ...
  const int clearance_cells = static_cast<int>(std::ceil(
    config_.coverage_path_clearance_meters / std::max(map.resolution,
  ...
  for (unsigned int iy = 0; iy < map.height_cells; ++iy) {
    for (unsigned int ix = 0; ix < map.width_cells; ++ix) {
      ...
      for (std::size_t waypoint_index = 1U; waypoint_index < corridor_waypoints.size();
        ++waypoint_index)
      {
...
: rclcpp::Node("vda5050_parser_node", options)
{
  ...
  missions_log_directory_ = declare_parameter<std::string>("missions_log_directory",
      "missions/logs");
  ...
  RCLCPP_INFO(
    get_logger(),
    "MissionParserNode watching %s, reading source JSON from %s, "
    "writing staged mission artifacts to %s, and %s discovered missions",
...
void MissionParserNode::buildDiscoveredMissionArtifacts()
{
  ...
  if (!std::filesystem::exists(missions_directory) ||
    !std::filesystem::is_directory(missions_directory))
  {
...
std::optional<std::filesystem::path> MissionParserNode::selectActiveMissionPath()
{
  if (!mission_path_.empty()) {
    ...
    const std::filesystem::path fallback_source =
      resolvePath(missions_directory_) /
      (configured_path.stem().string() + mission_file_extension_);
...
std::filesystem::path MissionParserNode::stageMissionFile(
  const std::filesystem::path & mission_path)
{
...
bool MissionParserNode::buildArtifactsForMission(const std::filesystem::path & mission_path)
{
  try {
    ...
    const std::filesystem::path legacy_image_path = missions_directory /
      (static_costmap_basename + ".pgm");
    const std::filesystem::path legacy_yaml_path = missions_directory /
      (static_costmap_basename + ".yaml");
    const std::filesystem::path legacy_coverage_path = missions_directory /
      (coverage_basename + ".geojson");
    ...
    RCLCPP_INFO(get_logger(), "Built mission artifacts from %s",
        staged_mission_path.string().c_str());
    return true;
  ...
  std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  ...
  response->message = success ?
    ("Built " +
    (mission_path_.empty() ? std::string("<auto-selected>") : resolveMissionPath().string())) :
    ("Failed to build " +
    (mission_path_.empty() ? std::string("<auto-selected>") : resolveMissionPath().string()));
}
...
MapExtent parseBounds(const nlohmann::json & bounds)
{
  ...
  throw std::runtime_error(
      "map_georeference bounds must be an object or [min_x,min_y,max_x,max_y]");
}
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<amr_sweeper_mission_builder::MissionParserNode>());
  rclcpp::shutdown();
  return 0;
}
