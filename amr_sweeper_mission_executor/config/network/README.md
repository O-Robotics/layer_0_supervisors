...
1. Copy [99-amr-sweeper-static.yaml](/mnt/c/home/dev/rob_ws/src/layer_0_supervisors/amr_sweeper_mission_executor/config/network/99-amr-sweeper-static.yaml) into `/etc/netplan/`.
2. Confirm the robot Wi-Fi interface name is `wlan0`.
3. Apply with `sudo netplan apply`.

## Notes

- The web server still binds to `0.0.0.0:8080`; the static IP determines how operators reach it.
