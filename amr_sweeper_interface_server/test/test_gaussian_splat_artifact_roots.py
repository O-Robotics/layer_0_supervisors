...
class UnifiedMissionPageTest(unittest.TestCase):
    ...
    def test_teleop_uses_full_viewport_control_shell(self) -> None:
        ...
        self.assertIn("LED work lights", html)
        self.assertNotIn("Front work lights", html)
        ...
        self.assertIn("function pulseStartButtons", html)
        self.assertIn("function stopPulsingStartButtons", html)
        self.assertIn("start-button-pulse", html)
        self.assertIn("0.95s ease-in-out infinite", html)
        self.assertIn("button.classList.add('needs-start')", html)
        self.assertIn("button.classList.remove('needs-start')", html)
    ...
    def test_primary_pages_use_shared_topbar_with_fsm_battery_and_drawer_connection(self) -> None:
        ...
        for html in pages:
            ...
            self.assertEqual(html.count('id="drawer-safety-stop-button"'), 1)
            self.assertIn("function requestSafetyStop", html)
            self.assertIn("'/api/v1/safety/stop'", html)
            self.assertIn("width: min(67.5vw, 292px);", html)
            self.assertIn("animation: drawer-live-pulse 1.2s infinite", html)
            self.assertIn("@keyframes drawer-live-pulse", html)
