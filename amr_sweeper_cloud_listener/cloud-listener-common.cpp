...
std::string required_environment_variable(const char * variable_name)
{
  ...
  if (value == nullptr || value[0] == '\0') {
    throw std::runtime_error(std::string(ERROR) + " " + std::string(variable_name) +
      " IS NOT SET. PROVIDE IT VIA /storage/secrets/.env");
  }
...
{
  return manifest_output_path_from_home().parent_path() / kMessageOutputFileName;
}
  ...
  const std::string & payload)
{
  ...
  if (!output_directory.empty()) {
    ...
    if (create_error) {
      throw std::runtime_error(
        std::string(ERROR) + " FAILED TO CREATE OUTPUT DIRECTORY: " +
  ...
  if (!output) {
    throw std::runtime_error(
      std::string(ERROR) + " FAILED TO OPEN OUTPUT FILE: " + output_path.string());
  }

  output.write(payload.data(), static_cast<std::streamsize>(payload.size()));
  if (!output.good()) {
    throw std::runtime_error(
      std::string(ERROR) + " FAILED TO WRITE OUTPUT FILE: " + output_path.string());
  }

  output.close();
  if (!output) {
    throw std::runtime_error(
      std::string(ERROR) + " FAILED TO FINALIZE OUTPUT FILE: " + output_path.string());
  }
}
